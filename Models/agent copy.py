...
class Agent:
    ...
    def action(self,posibilities,rewards):
        self.__memory_control(posibilities,rewards)
        if self.stock is None:
            for r,i,j in rewards:
    ...
    def action_backup(self,posibilities,rewards):
        self.__memory_control(posibilities,rewards)
        if self.stock is None:
            for r,i,j in rewards:
                result = self.__take(r)
    ...
    def __memory_control_backup(self,pos,rewards):
        for r,i,j in rewards:
            if len(self.memory) == self.memory_size:
                self.memory.pop()
